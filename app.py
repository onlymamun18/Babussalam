...
# আপনার নতুন দেওয়া SCRIPT URL
SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwXbh09w-Q7pJlrkxL-UqyMTBPdsJwlzYgT3FKbSusJJ3Wowk6SqwHQRCN7LsgEHWCu/exec"
...
# --- ২. প্রিমিয়াম UI ডিজাইন (অরিজিনাল) ---
st.markdown("""
    <style>
...
# --- ৪. মেনু নেভিগেশন (সবগুলো মেনু এখানে আছে) ---
...
# --- ২. প্রোফাইল সার্চ ---
    ...
    st.header("🔍 শিক্ষার্থীর তথ্য ও হাজিরা")
    ...
    if sid and df_s is not None:
        ...
        if not student.empty:
            ...
            if pin_check == ADMIN_PIN:
                ...
                st.info(f"🆔 আইডি: {s[0]}")
                st.warning("🔒 পূর্ণাঙ্গ তথ্য দেখতে অ্যাডমিন পিন দিন।")
...
# --- ৩. দৈনিক হাজিরা ---
        ...
        if df_s is not None:
            with st.form("att_form"):
                ...
                for _, row in df_s.iterrows():
                    status = st.selectbox(f"{row[1]} ({row[0]})", ["উপস্থিত", "অনুপস্থিত", "ছুটি"], key=f"att_{row[0]}")
    ...
    else: st.warning("হাজিরা ইনপুটের জন্য পিন দিন।")
...
# --- ৪. রেজাল্ট শিট ---
    ...
    if rid and df_r is not None:
        r = df_r[df_r.iloc[:, 0].str.strip() == rid]
        if not r.empty:
            st.success(f"ফলাফল পাওয়া গেছে: {r.iloc[0, 1]}")
            st.table(r.iloc[0])
        else: st.warning("এই আইডির কোনো রেজাল্ট পাওয়া যায়নি।")

# --- ৫. অ্যাডমিন প্যানেল ---
elif menu == "🔐 অ্যাডমিন প্যানেল":
    if st.sidebar.text_input("পিন:", type="password", key="adm_panel_p") == ADMIN_PIN:
        opt = st.selectbox("কাজ নির্বাচন করুন", ["নতুন ভর্তি", "ছাত্র তালিকা", "ডাটা ডিলিট"])
        if opt == "নতুন ভর্তি":
            with st.form("adm_full"):
                ...
                if st.form_submit_button("ভর্তি সম্পন্ন করুন"):
                    ...
                    st.success("ভর্তি সফল ও তথ্য সেভ হয়েছে!")
            ...
            if st.button("ডিলিট করুন"):
                ...
                st.success("ডাটা ডিলিট সম্পন্ন!")
    else: st.error("অ্যাডমিন পিন দিয়ে লগইন করুন।")
